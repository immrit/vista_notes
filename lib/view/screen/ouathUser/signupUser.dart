...
class SignUpScreen extends ConsumerWidget {
  // تغییر به ConsumerWidget
  const SignUpScreen({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final bool isLoading = ref.watch(isLoadingProvider); // استفاده از ref.watch
    final emailController = TextEditingController();
    final passController = TextEditingController();
    final confirmPasswordController = TextEditingController();
    bool redirecting = false; // متغیر محلی برای مدیریت هدایت
    ...
    Future<void> signUp() async {
      ref.read(isLoadingProvider.notifier).state =
          true; // به روزرسانی وضعیت بارگذاری
      try {
        ...
        ref.read(isLoadingProvider.notifier).state = false; // پایان بارگذاری
      }
    }
